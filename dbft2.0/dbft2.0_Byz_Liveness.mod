...
# Ensure circular behavior, if previous primary not found and conclusions not done we can not move on. Primary should had fault, at least
nextPrimaryOnlyIfPreviousPrimary{v in V: v>1}: sum{i in R} primary[i,v] <= sum{i in R} primary[i,v-1];
# You should proof you have certificates to be the primary, proof the changeviews message records of previous view
nextPrimaryOnlyIfEnoughCV{i in R, v in V: v>1}: primary[i,v] <= changeViewRecvPerNodeAndView[i,v-1]/M;
nextPrimaryOnlyIfBlockNotRelayedNonByz{i in R_OK, v in V: v>1}: primary[i,v] <= (1 - sum{t in T} sum{v2 in V:v2<v} BlockRelay[t, i,v2]);
atLeastOnePrimaryIfEnoughCV{i in R_OK, v in V: v>1}: sum{ii in R} primary[ii,v]*1000 >= changeViewRecvPerNodeAndView[i,v-1]/M - a[i];
/* ============== Primary constraints ============== */
...
prepReqOnlyOnceAndSentNonByz{i in R_OK, v in V}: sum{t in T} SendPrepReq[t,i,v] >= primary[i,v];
prepReqReceivedSendByJNonByz{t in T, i in R_OK, j in R_OK, v in V:j!=i}: RecvPrepReq[t,i,j,v] >= sum{t2 in T: t2<t} SendPrepReq[t2,j,v];
sendPrepReqOnlyIfChangeViewNotSentNonByz{i in R_OK, v in V, t in T: t>1}: SendPrepReq[t,i,v] <= (1 - sum{t2 in T: t2<t} SendCV[t2, i, v]);
sendPrepReqOnlyIfBlockNeverRelayedNonByz{i in R_OK, v in V: v>1}: sum{t in T} SendPrepReq[t,i,v] <= (1 - sum{t in T} sum{v2 in V:v2<v} BlockRelay[t, i, v2]);
sendPrepReqOnlyIfViewBeforeWasAccomplishedNonByz{i in R_OK, v in V: v>1}: sum{t in T} SendPrepReq[t,i,v] <= changeViewRecvPerNodeAndView[i,v-1]/M;
...
prepRespSendOptionally{t in T, i in R, v in V: t>1}: SendPrepResp[t,i,v] <= sum{t2 in T:t2<=t} sum{j in R} RecvPrepReq[t2,i,j,v];
...
prepRespSendOrNotIfYouAreByzantineNonByz{t in T, i in R_OK, v in V: t>1}: SendPrepResp[t,i,v]*1000 >= sum{t2 in T:t2<=t} sum{j in R_OK} RecvPrepReq[t2,i,j,v];
prepRespReceivedSendByJNonByz{t in T, i in R_OK, j in R_OK, v in V: t>1 and j!=i}: RecvPrepResp[t,i,j,v] >= sum{t2 in T: t2<t} SendPrepResp[t2,j,v];
sendPrepResponseOnlyIfChangeViewNotSentNonByz{i in R_OK, v in V, t in T: t>1}: SendPrepResp[t,i,v] <= (1 - sum{t2 in T: t2<t} SendCV[t2, i, v]);
sendPrepResponseOnlyIfBlockNeverRelayedNonByz{i in R_OK, v in V: v>1}: sum{t in T} SendPrepResp[t,i,v] <= (1 - sum{t in T} sum{v2 in V:v2<v} BlockRelay[t, i, v2]);
sendPrepResponseOnlyIfViewBeforeWasAccomplishedNonByz{i in R_OK, v in V: v>1}: sum{t in T} SendPrepResp[t,i,v] <= changeViewRecvPerNodeAndView[i,v-1]/M;
...
commitSentIfMPrepRespNonByz{t in T, i in R_OK, v in V:t>1}: SendCommit[t,i,v]*1000 >= (sum{t2 in T: t2<=t} sum{j in R_OK} RecvPrepResp[t2,i,j,v])/M + b;
commitReceivedSendByJNonByz{t in T, i in R_OK, j in R_OK, v in V: t>1 and j!=i}: RecvCommit[t,i,j,v] >= sum{t2 in T: t2<t} SendCommit[t2,j,v];
sendCommitOnlyIfChangeViewNotSentNonByz{i in R_OK, v in V, t in T: t>1}: SendCommit[t,i,v] <= (1 - sum{t2 in T: t2<t} SendCV[t2, i, v]);
sendCommitOnlyIfBlockNeverRelayedNonByz{i in R_OK, v in V: v>1}: sum{t in T} SendCommit[t,i,v] <= (1 - sum{t in T} sum{v2 in V:v2<v} BlockRelay[t, i, v2]);
...
blockRelayIfEnoughCommitsNonByz{t in T, i in R_OK, v in V: t>1}: BlockRelay[t, i, v]*1000 >= (sum{t2 in T: t2<=t} sum{j in R} RecvCommit[t2,i,j,v])/M + c;
...
receivedCVNonByz{t in T, i in R_OK, j in R_OK, v in V: t>1 and j!=i}: RecvCV[t,i,j,v] >= sum{t2 in T: t2<t} SendCV[t2,j,v];
...
sendCVOnlyIfBlockNeverRelayedNonByz{t in T, i in R_OK, v in V}: SendCV[t,i,v] <= 1 - sum{t2 in T:t2<=t} BlockRelay[t2,i,v];
...
sendCVOnlyIfCommitNotSentNonByz{t in T, i in R_OK, v in V}: SendCV[t,i,v] <= (1 - sum{t2 in T: t2<t} SendCommit[t2, i, v]);
...
forceCVIfPrepReqtNotSentv1NonByz{i in R_OK, v in V}: sum{t in T} SendCV[t,i,v] >= (1 - prepReqRecvPerNodeAndView[i,v]);
forceCVIfPrepReqtNotSentAllNonByz{i in R_OK, v in V: v>1}: (sum{t in T} SendCV[t,i,v] + (1 - sum{j in R} primary[j,v-1])) >= (1 - prepReqRecvPerNodeAndView[i,v]);
...
/* ============== Obj Function ============== */
minimize obj: totalBlockRelayed*-100 + (sum{i in R} a[i]*100000) + (b+c)*100000; 
