...
sendCommitOnlyIfChangeViewNotSent{i in R, v in V}: sum{t in T} SendCommit[t,i,v] <= (1 - sum{t in T} SendCV[t, i, v]);
sendCommitOnlyIfViewBeforeWasAccomplished{i in R, v in V: v>1}: sum{t in T} SendCommit[t,i,v] <= (sum{j in R} sum{t in T} RecvCV[t,i,j,v-1])/M;
...
sendCVOnlyIfCommitNotSent{i in R, v in V}: sum{t in T} SendCV[t,i,v] <= (1 - sum{t in T} SendCommit[t, i, v]);
